...
fn execute_lets_the_job_environment_override_the_workflow_one() {
    ...
    let response = GitHubJobEnvironmentAdapter::new().execute(
        BuildJobEnvironmentRequest::new(&workflow, &job_env(&[("MODE", "job")])),
    );
...
fn execute_sets_the_runners_own_variables() {
    ...
    let response = GitHubJobEnvironmentAdapter::new().execute(BuildJobEnvironmentRequest::new(&workflow, &HashMap::new()));
...
fn execute_keeps_a_job_declared_path() {
    ...
    let response = GitHubJobEnvironmentAdapter::new().execute(
        BuildJobEnvironmentRequest::new(&workflow, &job_env(&[("PATH", "/job/bin")])),
    );
