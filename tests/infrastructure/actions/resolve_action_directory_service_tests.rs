...
fn execute_resolves_a_local_reference_under_the_repository_root() {
    ...
    .execute(ResolveActionDirectoryRequest::new("./actions/greet", Path::new("/repo")))
    .unwrap();
...
fn execute_skips_a_checkout_action_because_the_workspace_is_mounted() {
    ...
    .execute(ResolveActionDirectoryRequest::new("actions/checkout@v4", Path::new("/repo")))
    .unwrap();
...
fn execute_reports_container_actions_as_unsupported() {
    ...
    .execute(ResolveActionDirectoryRequest::new("docker://node:20", Path::new("/repo")))
    .unwrap_err();
...
fn execute_delegates_a_remote_reference_to_the_fetcher() {
    ...
    let resolved = service(fetcher.clone())
        .execute(ResolveActionDirectoryRequest::new("https://data.forgejo.org/actions/cache@v4", Path::new("/repo")))
        .unwrap();
