...
fn execute_sets_the_action_path() {
    let response =
        GitHubActionInputEnvironmentAdapter::new().execute(BuildActionInputEnvironmentRequest::new(&HashMap::new(), &HashMap::new(), "/tmp/actions/greet"));
...
fn execute_exposes_inputs_as_upper_snake_case_variables() {
    let response =
        GitHubActionInputEnvironmentAdapter::new().execute(
