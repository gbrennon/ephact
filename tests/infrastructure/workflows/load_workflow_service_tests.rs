...
fn execute_errors_for_content_that_is_not_a_workflow_document() {
    let result = LoadWorkflowService::new().execute(LoadWorkflowRequest::new("- push\n- pull_request\n"));
...
fn execute_errors_for_malformed_yaml() {
    let result = LoadWorkflowService::new().execute(LoadWorkflowRequest::new("name: [unterminated\n",));
