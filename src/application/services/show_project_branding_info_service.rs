...
impl ShowProjectBrandingInfoPort for ShowProjectBrandingInfoService {
    fn execute(&self) -> Result<ShowProjectBrandingInfoResponse, Box<dyn Error>> {
        ...
        Ok(ShowProjectBrandingInfoResponse::new(branding.name().as_str().to_string(), branding.description().as_str().to_string(), branding.version().as_str().to_string(), branding.emblem().as_str().to_string()))
    }
}
