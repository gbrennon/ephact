...
impl CreateJobContainerPort for CreateJobContainerService {
    fn execute(
        ...
        let container_config = ContainerConfig::new(request.image().to_string(), None, HashMap::new(), vec![format!(
                "{}:{}:Z",
                request.repo_path().display(),
                CONTAINER_WORKSPACE
            )], Some(CONTAINER_WORKSPACE.into()), Some(vec!["sleep".into(), "infinity".into()]), None, None, Some(request.container_name().to_string()), RunnerContext::default());
