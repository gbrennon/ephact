...
impl PrepareJobContainerPort for PrepareJobContainerService {
    fn execute(
        ...
        let image = self.image_puller.execute(PullJobImageRequest::new(request.runs_on()))?;
        ...
        let container = self.container_creator.execute(CreateJobContainerRequest::new(&image, &container_name, &legacy_container_name, request.repo_path()))?;
